...
function CreatePost() {
  const {updatePost, filteredCategories, getSection, filteredTags, edit, setIsWaiting} = useDashboardContext()
      ...
      <div className='flex flex-row'>
        <div className='basis-8/12 m-5 py-5 px-4 bg-white rounded-sm shadow-sm text-left space-y-4'>
          ...
          <form action="" className='grid space-y-3'>
            ...
            <CKEditor editor={ClassicEditor} data={post.content} value={post.content} onChange={handleEditor} name="content" className='px-5 py-1 border border-slate-800 rounded-sm border-opacity-50'></CKEditor>
          </form>
        </div>
        <div className='grid grid-rows-3 basis-4/12 py-5 pr-4 space-y-5'>
          <div className='bg-white rounded-sm shadow-sm text-left space-y-7 px-4 py-5'>
            ...
            <div className='space-y-2'>
              <div className='grid grid-flow-col items-center'>
                <div className='flex text-xs space-x-1'>
                  ...
                  <select name="status" id="status" value={post.status} onChange={(e)=>{handleChange(e)}}>
              ...
              <div className='grid grid-flow-col items-center'>
                <div className='flex text-xs space-x-1'>
                  ...
                  {
                    post.categories.map((item, index)=>{return index !== post.categories.length -1 ? <span key={index} className='text-text-gray font-semibold'>{item}, </span> : <span key={index} className='text-text-gray font-semibold'>{item} </span> })
                  }
                </div>
              ...
              <div className='grid grid-flow-col items-center'>
                <div className='flex text-xs space-x-1'>
                  ...
                  {
                    post.tags.map((item, index)=>{return index !== post.tags.length -1 ? <span key={index} className='text-text-gray font-semibold'>{item}, </span> : <span key={index} className='text-text-gray font-semibold'>{item} </span> })
                  }
                </div>
                <div className='flex justify-end'>
            ...
            <div className='grid grid-flow-col'>
              <button disabled={setIsWaiting} type='submit' className='px-4 py-1 font-bold bg-kb w-fit text-white rounded-sm text-sm' onClick={(e)=>handleSubmit(e, "publish")}>Publish</button>
